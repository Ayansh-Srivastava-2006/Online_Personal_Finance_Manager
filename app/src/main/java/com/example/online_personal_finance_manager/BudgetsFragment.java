                                ...
                                if (getActivity() != null) {
                                    getActivity()
                                            .runOnUiThread(
                                                    () -> Toast
                                                            .makeText(getContext(),
                                                                    getString(R.string.error_adding_budget,
                                                                            e.getMessage()),
                                                                    Toast.LENGTH_SHORT)
                                                            .show());
                                }
    ...
    private void loadBudgets() {
        FinanceManager.getInstance().getBudgets(new FinanceManager.CustomCallback<List<Budget>>() {
            ...
            public void onResult(List<Budget> result) {
                if (getActivity() != null)
                    getActivity().runOnUiThread(() -> displayBudgets(result));
            }
            ...
            public void onError(Exception e) {
                if (getActivity() != null) {
                    getActivity().runOnUiThread(() -> Toast.makeText(getContext(),
                            getString(R.string.error_loading_budgets, e.getMessage()), Toast.LENGTH_SHORT).show());
                }
            }
        });
    }
    ...
    private void addBudgetCard(Budget budget) {
        ...
        if (progress > 100)
            pb.getProgressDrawable()
                    .setColorFilter(new PorterDuffColorFilter(Color.RED, android.graphics.PorterDuff.Mode.SRC_IN));
        else if (progress > 80)
            pb.getProgressDrawable().setColorFilter(
                    new PorterDuffColorFilter(Color.rgb(255, 165, 0), android.graphics.PorterDuff.Mode.SRC_IN));
        else
            pb.getProgressDrawable().setColorFilter(
                    new PorterDuffColorFilter(Color.rgb(46, 204, 113), android.graphics.PorterDuff.Mode.SRC_IN));
        ...
        builder.setView(layout)
                ...
                .setPositiveButton(R.string.save, (dialog, id) -> {
                    ...
                    if (!category.isEmpty() && !amountStr.isEmpty()) {
                        ...
                        FinanceManager.getInstance().updateBudget(updatedBudget,
                                new FinanceManager.CustomCallback<Boolean>() {
                                    @Override
                                    public void onResult(Boolean result) {
                                        if (getActivity() != null) {
                                            getActivity().runOnUiThread(() -> {
                                                Toast.makeText(getContext(), R.string.budget_updated,
                                    ...
                                    public void onError(Exception e) {
                                        if (getActivity() != null) {
                                            getActivity()
                                                    .runOnUiThread(() -> Toast
                                                            .makeText(getContext(),
                                                                    getString(R.string.error_updating_budget,
                                                                            e.getMessage()),
                                                                    Toast.LENGTH_SHORT)
                                                            .show());
                                        }
    ...
    private void deleteBudget(String budgetId) {
        FinanceManager.getInstance().deleteBudget(budgetId, new FinanceManager.CustomCallback<Boolean>() {
            ...
            public void onError(Exception e) {
                if (getActivity() != null) {
                    getActivity().runOnUiThread(() -> Toast.makeText(getContext(),
                            getString(R.string.error_deleting_budget, e.getMessage()), Toast.LENGTH_SHORT).show());
                }
            }
        });
    }
